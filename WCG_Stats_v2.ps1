...
$Header  = "Name,Joined,Years,Days,Hours,Minutes,Seconds,Points,Results,RecDate,Placement,Retired"
...
#$File = "Z:\PowerShell\WCG\2020-06-12.txt"
...
ForEach ($R in $Records) {
  ...
  If ($Line -ne 0) {
    ...
    If ($Rec[1] -ne "") {
      ...
      If ($Rec[1].Contains("[") -and $Rec[1].Contains("]")) {
        $Retired = "Yes"
      } Else {
        $Retired = "No"
      }
...
$Array | Select-Object Name,Years,Days,Hours,Minutes,Seconds,Points,Results,Placement,Retired | ft -AutoSize
